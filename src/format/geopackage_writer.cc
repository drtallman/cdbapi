...
auto GeoPackageWriter::WriteTile(
  ...
  if (!driver) {
    return std::unexpected(
        Error(ErrorCode::kGdalError, "GPKG driver not available"));
  }

  char** options = nullptr;
  options = CSLSetNameValue(options, "TILE_FORMAT", "PNG");
  ...
  if (!dataset) {
    return std::unexpected(
        Error(ErrorCode::kGdalError,
              std::format("Failed to create GeoPackage: {}", path_str)));
  }
